...
function concatenar(codigoDigitado){
  let concatenado = '';
  codigoDigitado.forEach((v) => {
    concatenado += v.value;
  });
  return concatenado;
}
async function validarC(){
  const email = document.querySelector('#email').value;
  const codigoDigitado = document.querySelectorAll('.code-input');

    console.log(email, codigo);
    const re = await fetch('/validarC', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ emailDestino: email, codigoEnviado: concatenar(codigoDigitado) })
    });
